...
export default {
  ...
  state: {
    sidebarClose: false,
    sidebarStatic: true,
    sidebarColorName: config.app.themeColors[3][0],
    navbarColorName: config.app.themeColors[3][0],
  ...
  mutations: {
    ...
    toggleSidebar(state) {
      ...
      sessionStorage.sidebarStatic = nextState;
  ...
  actions: {
    ...
    getPermission({commit}) {    
      let _permit = JSON.parse(sessionStorage.getItem('permit'))
      commit('getPermission', _permit); 
    }
  },
};
