<template lang="pug">
  .dailyBoardGameRevenue
    widget.mb-4(:title="`<h4 class='fw-semi-bold'>${$t('開桌營收報表 (日)')}</h4>`" customHeader :fetchingData="false")
      .row.mt-4.px-3
        .form-group.col-12.col-md-3.px-2
          label(for="txtMonth") {{ $t('月份') }}
          span.warn-text *
          date-picker#txtMonth(
            type="month" v-model="dateMonth" valueType="format"
    ...
    template(v-else)
      .mt-3.mb-n4.px-0(xs="12" v-if="chartData && chartData.length >= 1")
        line-chart(:data="chartData" chartHeight="500")
      
      widget.mb-3(:fetchingData="loading")
...
export default {
    ...
    methods: {
      async EXPORT_REPORT() {
        ...
        let fileName = i18n.t('開桌營收報表 (日)') + '_' + moment(this.dateMonth).format('YYYY-MM') + '_' + process.env.NODE_ENV +'.xlsx'
      ...
      async getBoardGameRevenue() {
        const body = {
          data: {
            ...
            start_date: moment(this.dateMonth).startOf('month').format('YYYY-MM-DD'),
            end_date: moment(this.dateMonth).endOf('month').format('YYYY-MM-DD'),
            date_type: 3,
      ...
      async search() {
        if(this.invalid.dateMonth) return
    ...
    async mounted() {
      this.dateMonth = moment().format(this.dateFormat)
...
<style lang="scss">
  .dailyBoardGameRevenue .VueTables__sort-icon::before {
    display: none;
  }

  .dailyBoardGameRevenue tr:first-child td:first-child {  
    pointer-events: none;
  }

  .dailyBoardGameRevenue .VueTables__row td:first-child{  
    min-width: 30px;
  }

  .dailyBoardGameRevenue .VueTables__child-row {
    > td:first-child {
      padding: 0;
    }
    thead {
  ...
  .dailyBoardGameRevenue tr:first-child .VueTables__child-row-toggler  {
    display: none;
  }
  
  .dailyBoardGameRevenue .table-hover tbody tr:hover {
    background-color: white;
  }

  .dailyBoardGameRevenue .VueTables__row:hover {
  ...
  .dailyBoardGameRevenue .VueTables__table th,
  .dailyBoardGameRevenue .VueTables__row td {
    &:nth-last-child(1) {
      max-width: 16%;
      min-width: 16%;
      width: 16%;
