<template>
  <div class="reportDaily">
    <widget class="mb-4" :title="`<h4 class='fw-semi-bold'>${$t('輸贏報表 (日)')}</h4>`" customHeader>
    ...
    <template v-else>
      ...
      <widget class="mb-4" :fetchingData="loading">
        ...
        <v-client-table v-if="refundRecordDataHiden && refundRecordData && refundRecordData.length>0" name="tbReport" :columns="colReportTwo" :options="optReport" :data="refundRecordData" ref="tbReport">
          <template v-if="hasDetail" v-slot:child_row="props">
            <v-client-table name="tbReport" :columns="colReportChildTwo" :options="optReport" :data="props.row.detail" ref="tbReport">
            </v-client-table>
          </template>
        </v-client-table>
...
import { getOperatorList, getGameList, getDailyReport, addVendorLog } from '@/services/service'
...
export default {
    ...
    methods: {
      ...
      async EXPORT_REPORT() {
        ...
        fileName = i18n.t('輸贏報表 (日)') + '_' + moment(this.dateYear).format('YYYY') + '_' + process.env.NODE_ENV +'.xlsx'
      ...
      setExcelListData() {
        ...
        this.excelData.forEach(item => { 
          // 視訊不顯示 服務費 彩金
          if(this.vendorCode === 'chess') delete item['tax_amount']
          else {
            delete item['service_fee']
            delete item['jackpot']
          }
      ...
      setChartData() {
        ...
        if(!this.merchantId || !this.gameId) {
          ...
          _group.forEach(_info => {
            ...
            this.recordData.forEach(_item => {
              if(_item.merchant_id == _info.merchant_id && _item.game_code == _info.game_code) {
      ...
      async getDailyReport() {
        ...
        try {
          let temp = await getDailyReport(body)
          if (temp.code !== 1000) {
            this.SHOW_MESSAGE(temp.code, Message[temp.code])
          } else {
            this.apiData = temp
        ...
        switch(sessionStorage.getItem('vendor_code')) {
          ...
          case 'chess':
            ...
            this.colReport_child = [
              'space', 'date', 'merchant_id', 'game_code', 'currency', 
              'total_round', 'total_count', 'total_bet', 'total_win', 'win_lose', 
              'service_fee', 'jackpot', 'rtp', 'total_people', 'average_round_per_people', 
              'average_bet_per_round'
            ]
            ...
            this.colReportChildTwo = [
              'space', 'date', 'merchant_id', 'game_code', 
              'currency', 'total_count', 'total_bet', 'total_win', 'win_lose', 
              'service_fee', 'jackpot', 'rtp'
            ]
...
<style lang="scss">
  .reportDaily .VueTables__sort-icon::before {
    display: none;
  }

  .reportDaily tr:first-child td:first-child {  
    pointer-events: none;
  }

  .reportDaily .VueTables__child-row {
    > td:first-child {
      padding: 0;
    }
    thead {
  ...
  .reportDaily tr:first-child .VueTables__child-row-toggler  {
    display: none;
  }
  
  .reportDaily .table-hover tbody tr:hover {
    background-color: white;
  }

  .reportDaily .VueTables__row:hover {
