<template lang="pug">
  .newAndOldMonthly
    widget.mb-4(:title="`<h4 class='fw-semi-bold'>${$t('新舊會員押注統計 (月)')}</h4>`" customHeader)
      .row.mt-4.px-3
...
import { getMonthlyNAOBettingCount, addVendorLog } from '@/services/service'
...
export default {
  ...
  methods: {
    ...
    async searchReport() {
      ...
      this.init()
      await this.getMonthlyNAOBettingCount()
      this.setChartData()
      this.dataFormat()
    ...
    async getMonthlyNAOBettingCount() {
      const body = {
        data: {        
          year: this.dateYear,
      ...
      try {
        let temp = await getMonthlyNAOBettingCount(body)
        if (temp.code !== 1000) {
      ...
      const body1 = {
        data: {                                 
          ...
          'action': 'select',
          'route': '/api/getMonthlyNAOBettingCount',
    ...
    setChartData() {
      ...
      tempList.forEach(item => {
        item.details?.forEach(node => {
          ...
          if(temp) this.chartData.push({ mixedData: [], arppuData: [], name: node.OperatorName })
        })
