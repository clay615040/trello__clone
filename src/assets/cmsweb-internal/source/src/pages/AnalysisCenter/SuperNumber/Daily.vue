<template lang="pug">
  .superNumberDaily
    widget.mb-4(:title="`<h4><span class='fw-semi-bold'>${$t('遊戲超級號碼熱門區 (日)')}</span></h4>`" customHeader)
      .row.mt-4.px-3
        .form-group.col-12.col-md-3.px-2
          label(for="txtMonth") {{ $t('月份') }}
          span.warn-text *
          date-picker#txtMonth(type="month" valueType="format" :lang="datePickerLang" v-model="dateMonth" :format="dateFormat" :change="CHANGE_DATE()" :input-class="{ 'form-control': true, 'is-invalid': invalid.dateMonth }" :placeholder='defaultChangeText' width="100%")
...
import { getOperatorList, getGameList, getSuperNumberBetList, getDailySuperNumber, addVendorLog } from '@/services/service'
...
export default {
  ...
  data() {
    return {
      colReport: ['date', 'merchant_id', 'game_code', 'bet_area', 'bet_count', 'superOdds1_count', 'superOdds2_count', 'superOdds3_count', 'superOdds4_count', 'superOdds5_count'],
      childColReport: ['space', 'date', 'merchant_id', 'game_code', 'bet_area', 'bet_count', 'superOdds1_count', 'superOdds2_count', 'superOdds3_count', 'superOdds4_count', 'superOdds5_count'],
      optReport: {
        perPage: 500,
        perPageValues: [50],
  ...
  methods: {
    ...
    async getSuperNumberData() {
      ...
      try {
        let temp = await getDailySuperNumber(body)
          ...
          item.detail.forEach(node => {
            for (let i = 0; i <= this.chartData.length; i++) {
              if(info.name === node.bet_area) {
                info.data.push({x: item.date, y: parseInt(node.superOdds1_count) + parseInt(node.superOdds2_count) + parseInt(node.superOdds3_count) + parseInt(node.superOdds4_count) + parseInt(node.superOdds5_count)})
                break
              }
            }
          })
...
<style lang="scss">
  .superNumberDaily .chartStyle {
    overflow-x: scroll;
  }

  .superNumberDaily .widgetControls {
    display: none;
  }

  .superNumberDaily .VueTables__sort-icon::before {
    display: none;
  }
  ...
  .superNumberDaily .VueTables__child-row {
    > td:first-child {
      padding: 0;
    }
    thead {
  ...
  .superNumberDaily .VueTables__row td{
    &:nth-last-child(1) {
      max-width: 9%;
      min-width: 9%;
      width: 9%;
  ...
  .superNumberDaily .space {
    width: 5%;
    height: 10px;
  }

  .superNumberDaily .bottom span {
    width: 20%;
  }

  .superNumberDaily .month {
    width: 8%;
  }

  .superNumberDaily .betArea {
    width: 12%;
  }

  .superNumberDaily .merchantId,
  .superNumberDaily .gameCode,
  .superNumberDaily .betCount {
    width: 10%;
  }
  ...
  .superNumberDaily .table-hover tbody tr:hover {
    background-color: white;
  }

  .superNumberDaily .VueTables__row:hover {
    background-color:rgba(0, 0, 0, 0.075) !important; 
  }

  .superNumberDaily .vue-treeselect__control {
    border: 1px solid #c1ccd3;
    border-radius: 0.5rem;
    max-height: 33.5px;
  }

  .superNumberDaily #selPermission.error {
  ...
  .superNumberDaily .vue-treeselect__placeholder {
    font-size: 1rem;
    font-weight: 300;
    color: #495057;
  }

  .superNumberDaily .tableTitle {
    display: flex;
    align-items: center;
  ...
  .superNumberDaily .superNumber {
    width: 45%;
  }

  .superNumberDaily .top {
    padding-top: 10.5px;
    display: flex;
    justify-content: center;
  }

  .superNumberDaily .bottom {
    display: flex;
  }

  .superNumberDaily thead {
    display: none;
  }
