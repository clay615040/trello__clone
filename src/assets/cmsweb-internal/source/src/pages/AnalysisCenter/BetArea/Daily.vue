<template lang="pug">
  .betAreaDaily
    widget.mb-4(:title="`<h4 class='fw-semi-bold'>${$t('遊戲下注區熱門度 (日)')}</h4>`" customHeader)
      .row.mt-4.px-3
...
export default {
  ...
  data() {
    return {
      colReport: ['date', 'merchant_id', 'game_code', 'bet_area', 'bet_count', 'win_count', 'total_bet', 'total_win', 'win_lose'],
      childColReport: ['space', 'date', 'merchant_id', 'game_code', 'bet_area', 'bet_count', 'win_count', 'total_bet', 'total_win', 'win_lose'],
      optReport: {
        perPage: 500,
        perPageValues: [50],
  ...
  methods: {
    ...
    getBetAreaData() {
      return new Promise(async(resolve, reject) => {                
        ...
        let _res = await axios.post('/api/dailyBetAreaPopular', { 'data':_data, 'hash':_hash })
          ...
          item.detail.forEach(node => {
            for (let i = 0; i <= this.chartData.length; i++) {
              if(info.name === node.bet_area) {
                info.data.push({x: item.date, y: removeComma(node.bet_count)})
                break
              }
            }
          })
...
<style lang="scss">
  .betAreaDaily .chartStyle {
    overflow-x: scroll;
  }

  .betAreaDaily .widgetControls {
    display: none;
  }

  .betAreaDaily .VueTables__sort-icon::before {
    display: none;
  }
  ...
  .betAreaDaily .VueTables__child-row {
    > td:first-child {
      padding: 0;
    }
    thead {
  ...
  .betAreaDaily tr th,
  .betAreaDaily .VueTables__row td{
    &:nth-last-child(1) {
      max-width: 13%;
      min-width: 13%;
      width: 13%;
  ...
  .betAreaDaily .table-hover tbody tr:hover {
    background-color: white;
  }

  .betAreaDaily .VueTables__row:hover {
    background-color:rgba(0, 0, 0, 0.075) !important; 
  }

  .betAreaDaily .vue-treeselect__control {
    border: 1px solid #c1ccd3;
    border-radius: 0.5rem;
    max-height: 33.5px;
  }

  .betAreaDaily #selPermission.error {
