<template lang="pug">
  .activeUserDaily
    widget.mb-4(:title="`<h4 class='fw-semi-bold'>${$t('日活躍用戶 (DAU)')}</h4>`" customHeader)
      .row.mt-4.px-3
...
import { getDailyActiveUser, addVendorLog } from '@/services/service'
...
export default {
  ...
  methods: {
    ...
    async getDailyActiveUser() {
      ...
      try {
        let temp = await getDailyActiveUser(body)
...
<style lang="scss">
  .activeUserDaily tr:first-child .VueTables__child-row-toggler  {
    display: none;
  }

  .activeUserDaily .VueTables__sort-icon::before {
    display: none;
  }

  .activeUserDaily .chartStyle {
    overflow-x: scroll;
  }

  .activeUserDaily .widgetControls {
    display: none;
  }
  ...
  .activeUserDaily .fontWeight {
    font-weight: 600;
  }

  .activeUserDaily .VueTables__child-row {
    > td:first-child {
      padding: 0;
    }
    thead {
  ...
  .activeUserDaily tr th,
  .activeUserDaily .VueTables__row td{
    &:nth-last-child(1) {
      max-width: 25%;
      min-width: 25%;
      width: 25%;
  ...
  .activeUserDaily .table-hover tbody tr:hover {
    background-color: white;
  }

  .activeUserDaily .VueTables__row:hover {
    background-color:rgba(0, 0, 0, 0.075) !important; 
  }

  .activeUserDaily .vue-treeselect__control {
    border: 1px solid #c1ccd3;
    border-radius: 0.5rem;
    max-height: 33.5px;
  }

  .activeUserDaily #selPermission.error {
  ...
  .activeUserDaily .vue-treeselect__placeholder {
    font-size: 1rem;
    font-weight: 300;
    color: #495057;
  }

  .activeUserDaily .chartClass {
    width: 100%;
  }
