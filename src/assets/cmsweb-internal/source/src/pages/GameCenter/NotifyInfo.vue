<template>
  <div>
    <widget v-if="!modeEditor" :title="`<h4><span class='fw-semi-bold'>${$t('通報綁定 - 列表')}</span></h4>`" :fetchingData="procSelect" customHeader>
      <div class="mt-4">
        ...
        <v-client-table :columns="colNotify" :options="optNotify" :data="resDataInfo">
    ...
    <widget v-if="modeEditor" :title="titleModify" customHeader>
      <div class="mt-4">
        <b-form-group label-cols="4" label-cols-lg="2" label-for="selGroup">
          <div slot="label">{{ $t('群組名稱') }}<span class="warn-text">*</span></div>
          <b-form-select id="selGroup" v-model="groupId" ref="groupId"
                          :class="{ 'is-invalid': invalid.groupId }">
            <option v-for="(item, key) in listGroup" :key="key" :value="item.group_id" :selected="groupId">
              {{ item.group_name }} ({{ item.pipeline_name }})
            </option>
          </b-form-select>
        </b-form-group>
        <b-form-group label-cols="4" label-cols-lg="2" label-for="selRule">
          <div slot="label">{{ $t('規則名稱') }}<span class="warn-text">*</span></div>
          <b-form-select id="selRule" v-model="ruleId" ref="ruleId"
                          :class="{ 'is-invalid': invalid.ruleId }">
            <option v-for="(item, key) in listRule" :key="key" :value="item.rule_id" :selected="ruleId">
              {{ item.rule_name }}
            </option>
          </b-form-select>
        </b-form-group>
...
export default {
  ...
  data() {
    return {
      colNotify: ['pipeline', 'group_name', 'group_code', 'rule_name', 'rule_code', 'active', 'action'],
      optNotify: {
        perPage: 10,
  ...
  async mounted() {
    ...
    _err = await this.REQUEST_GROUP_LIST();
    if(_err) {
      this.SHOW_MESSAGE(_err, Message[_err]);
      return;
    }

    _err = await this.REQUEST_RULE_LIST();
    if(_err) {
      this.SHOW_MESSAGE(_err, Message[_err]);
      return;
    }
  ...
  computed: {
    ...
    ...mapState('notifyGroup', {
      'procGroup': state => state.process.select,
      'listGroup': state => state.response.list,
    }),
    ...mapState('notifyRule', {
      'procRule': state => state.process.select,
      'listRule': state => state.response.list,
    }),
    
    titleModify() {
      return (this.modeEditor == 'create') ? `<h4><span class='fw-semi-bold'>${i18n.t('通報綁定 - 新增')}</span></h4>` : `<h4><span class='fw-semi-bold'>${i18n.t('通報綁定 - 修改')}</span></h4>`
    },
  },
  ...
  methods: {
    ...
    ...mapActions('notifyGroup', {        
      'REQUEST_GROUP_LIST': 'request_list',        
    }),
    ...mapActions('notifyRule', {        
      'REQUEST_RULE_LIST': 'request_list',        
    }),
