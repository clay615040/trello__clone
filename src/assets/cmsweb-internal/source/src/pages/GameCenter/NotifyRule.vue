<template>
  <div>
    <widget v-if="!modeEditor" :title="`<h4><span class='fw-semi-bold'>${$t('通報規則 - 列表')}</span></h4>`" :fetchingData="procSelect" customHeader>
      <div class="mt-4">
        ...
        <v-client-table :columns="colRule" :options="optRule" :data="resDataInfo">
    ...
    <widget v-if="modeEditor" :title="titleModify" customHeader>
      <div class="mt-4">
        <b-form-group label-cols="4" label-cols-lg="2" label-for="selOperator">
          <div slot="label">{{ $t('平台商') }}<span class="warn-text">*</span></div>
          <b-form-select id="selOperator" v-model="operatorId" ref="operatorId"
                          :class="{ 'is-invalid': invalid.operatorId }">
            <option v-for="(item, key) in listOperator" :key="key" :value="item.id" :selected="operatorId">
              {{ item.name }}
            </option>
          </b-form-select>
        </b-form-group>
        <b-form-group label-cols="4" label-cols-lg="2" label-for="txtRuleName">
          <div slot="label">{{ $t('規則名稱') }}<span class="warn-text">*</span></div>
          <b-form-input id="txtRuleName" v-model="ruleName" ref="ruleName"
                        :class="{ 'is-invalid': invalid.ruleName }"></b-form-input>
        </b-form-group>
        <b-form-group label-cols="4" label-cols-lg="2" label-for="txtRuleCode">
          <div slot="label">{{ $t('規則代碼') }}<span class="warn-text">*</span></div>
          <b-form-input id="txtRuleCode" v-model="ruleCode" ref="ruleCode"
                        :class="{ 'is-invalid': invalid.ruleCode }"></b-form-input>
        </b-form-group>
        <b-form-group label-cols="4" label-cols-lg="2" label-for="txtRuleDesc">
          <div slot="label">{{ $t('規則描述') }}<span class="warn-text">*</span></div>
          <b-form-input id="txtRuleDesc" v-model="ruleDesc" ref="ruleDesc"
                        :class="{ 'is-invalid': invalid.ruleDesc }"></b-form-input>
        </b-form-group>
...
export default {
  ...
  data() {
    return {
      colRule: ['operator_name', 'rule_name', 'rule_code', 'rule_desc', 'active', 'action'],
      optRule: {
        perPage: 10,
        ...
        headings: {
          ...
          'rule_name': () => i18n.t('規則名稱'),
          'rule_code': () => i18n.t('規則代碼'),
          'rule_desc': () => i18n.t('規則描述'),
          'active': () => i18n.t('啟用狀態'),
          'action': () => i18n.t('操作'),
        },
      },
  ...
  async mounted() {
    ...
    _err = await this.REQUEST_OPERATOR_LIST();
    if(_err) {
  ...
  computed: {
    ...
    ...mapGetters('operator', {
      'listOperator': 'list_filter',
    }),     
    
    titleModify() {
      return (this.modeEditor == 'create') ? `<h4><span class='fw-semi-bold'>${i18n.t('通報規則 - 新增')}</span></h4>` : `<h4><span class='fw-semi-bold'>${i18n.t('通報規則 - 修改')}</span></h4>`
    },
  ...
  methods: {
    ...
    ...mapActions('operator', {
      'REQUEST_OPERATOR_LIST': 'request_list'
    }),
    async SELECT_DATA() {
    ...
    async UPDATE_DATA(data) {
      ...
      this.ruleCode = data.rule_code
      this.ruleDesc = data.rule_desc
    ...
    active: {
      handler: function(newValue, oldValue) {
        if(newValue == 'disable' && !this.active_confirm && this.modeEditor !== 'create') {
          this.message = i18n.t('通報綁定如有綁定此規則，則該通報也將一併停用，請確認是否繼續停用 ?')
