<template>
  <div>
    <widget class="mb-4" :title="`<h4 class='fw-semi-bold'>${$t('API 傳遞紀錄')}</h4>`" customHeader>
      <div class="row mt-4 px-3">
        <div class="form-group col-12 col-md-4 px-2">
          <label for="txtTransactionId">{{ $t('交易序號') }}</label>
        ...
        <div class="form-group col-12 col-md-4 px-2">
            <label for="txtUserId">{{ $t('使用者 ID') }}</label>
      ...
      <div class="row px-3">
        <div class="form-group col-12 col-md-3 px-2">
          ...
          <select id="selOperator" class="form-control" v-model="merchantId" @change="getGameList()" :class="{ 'is-invalid': invalid.operator }">
            <option v-for="(value, key) in operatorList" :key="key" :value="value.code">{{ value.name }}</option>
          </select>
        </div>
        <div class="form-group col-12 col-md-3 px-2">
            ...
            <select id="selGameId" class="form-control" v-model="gameId">
              <option v-for="(value, key) in gameList" :key="key" :value="value.game_code">{{ value.game_name_lang }}
                <span v-if="value && value.game_name_lang!=='All'">({{ value.game_code }})</span>
              </option>
            </select>
        </div>
    ...
    <widget class="mb-3" v-else>
      ...
      <v-client-table name="tbRecord" :columns="colRecord" :options="optRecord" 
        ...
        <template v-slot:child_row="props">
          ...
          <div><strong>{{ $t('使用者 ID') }} : </strong>{{ props.row.user_id }}</div>
          <div><strong>{{ $t('遊戲代碼') }} : </strong>{{ props.row.game_id }}</div>
          <div><strong>{{ $t('系統局號') }} : </strong>{{ props.row.round_id }}</div>
          <div><strong>{{ $t('投注') }} : </strong>{{ props.row.bet_amount }}</div>
          <div><strong>{{ $t('贏分') }} : </strong>{{ props.row.win_amount }}</div>
          <div><strong>{{ $t('balance') }} : </strong>{{ props.row.balance }}</div>
          <div><strong>{{ $t('幣別') }} : </strong>{{ props.row.currency }}</div>
          <div><strong>{{ $t('細節') }} : </strong>{{ props.row.detail }}</div>
          <div><strong>{{ $t('回應內容') }} : </strong>
            <span v-if="props.row.response_data">
              <button type="button" class="btn btn-link border-0 py-0 px-2" @click="COPY_TEXT(props.row.response_data)" >
                <i class="fa fa-clone" aria-hidden="true"></i>
              </button>
              <json-view :data="JSON.parse(props.row.response_data)" rootKey="" :styles="{ arrowSize: '1px' }"/>
            </span>
          </div>
        </template>        
      </v-client-table>
...
export default {
    ...
    data() {
      return {
        colRecord: ['type', 'url', 'merchant_id', 'vendor_name', 'transaction_id', 'create_date'],
        optRecord: {
          perPage: 50,
          perPageValues: [50],
          ...
          rowClassCallback(row) {
            if(!row.response_data) return '';
            return JSON.parse(row.response_data).code != 0? 'warn-bg': '';
          }
        },
        pageNo: 1,
        pageSize: 50,
    ...
    methods: {
      ...
      async QUERY_RECORD(isReload) {
        ...
        if(_diffTime > 3600* 1000* 24* 70) {
          this.overTime = true
          this.overTimeText = i18n.t('不能超過 70 天')
          return
        }
      ...
      CHANGE_PAGE_RECORD() {
        ...
        this.transactionId = this.reqRecord.transaction_id;
        this.roundId = this.reqRecord.round_id;
        this.userId = this.reqRecord.user_id;
